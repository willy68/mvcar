...
class SQLBuilder
{
	...
	public function where(/* (conditions, values) || (hash) */)
	{
		...
		if($this->where)
			$append = true;
		$this->apply_where_conditions(func_get_args(), $append, ' AND ', $this->op);
		return $this;
	}

	public function or_where(/* (conditions, values) || (hash) */)
	{
		$append = false;
		if($this->where)
			$append = true;
		$this->apply_where_conditions(func_get_args(), $append, ' OR ', $this->op);
		return $this;
	}

	public function order($order,$sens = 'ASC')
	{
		$this->order = $order.' '.$sens;
		return $this;
	}
	...
	public function select($select,$quote=true)
	{
		...
		{		
			if($quote == true)
			{
				...
				$ar_select = explode(",",$select);
				$ar_quote = array();
				foreach($ar_select as $value)
				{
					$ar_quote[] = $this->quote_name($value);
				}
		
				$this->select = implode(",",$ar_quote);
			}
			else
				$this->select = $select;
		}
	...
	public static function create_conditions_from_underscored_string(Connection $connection, $name, &$values=array(), &$map=null)
	{
		...
		for ($i=0,$j=0,$n=count($parts); $i<$n; $i+=2,++$j)
		{
			...
			$conditions[0] .= $this->quote_name($name) . $bind;
		}
		return $conditions;
	}
	...
	{
		$new = array();
		$table = $this->quote_name($this->table);

		foreach ($hash as $key => $value)
		{
			$k = $this->quote_name($key);
			$new[$table.'.'.$k] = $value;
		}
	...
	private function apply_where_conditions($args, $append = false, $log = ' AND ', $op = '=')
	{
		$num_args = count($args);
		...
		if ($num_args == 1 && is_hash($args[0]))
		{
			...
			$e = new Expressions($this->connection,$hash,$log,$op);
			$this->where .= ($append == true) ? $log.$e->to_s() : $e->to_s();
			$this->where_values = ($append == true) ? array_merge($this->where_values,array_flatten($e->values()))
		...
		elseif ($num_args > 0)
		{
			...
			foreach ($values as $name => &$value)
			{
				if (is_array($value))
				{
					$e = new Expressions($this->connection,$args[0],$log,$op);
			...
			$this->where .= ($append == true) ? $log.$args[0] : $args[0];
			$this->where_values = ($append == true) ? array_merge($this->where_values, $values) : $values;
		}
	}
	...
	*/		if ($this->order)
			...
			if ($this->limit)
				$sql = $this->apply_limit($sql,null,$this->limit);
	/*	}
	*/
		return $sql;
	}
	...
	private function build_insert()
	{
		...
		{
			$sql =
				"INSERT INTO $this->table($keys," . $this->quote_name($this->sequence[0]) .
	...
	private function build_select()
	{
		...
		if ($this->limit || $this->offset)
			$sql = $this->apply_limit($sql,$this->offset,$this->limit);
	...
	public function apply_limit($sql, $offset, $limit)
	{
